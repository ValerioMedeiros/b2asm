...
THEORY ListVariablesX IS
  ...
  List_Variables(Implementation(assignmentasmw))==(ucend,ucpc,ucmem);
  External_List_Variables(Implementation(assignmentasmw))==(uc.end,uc.pc,uc.mem)
END
&
THEORY ListVisibleVariablesX IS
  ...
  External_List_VisibleVariables(Implementation(assignmentasmw))==(?);
  Expanded_List_VisibleVariables(Implementation(assignmentasmw))==(?);
...
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Implementation(assignmentasmw),run)==(btrue | @(pc,end).(pc:=0;end:=1;(end : NATURAL | ucpc,ucend:=0,end);WHILE pc<end DO pc = 0 ==> (0 : NATURAL & 0 : uint32 & ucpc+1<=ucend | ucmem,ucpc:=ucmem<+{0|->0},ucpc+1) [] not(pc = 0) ==> skip;(btrue | pc:=ucpc) INVARIANT 0<=pc & pc<=end & pc = ucpc & end = ucend & (pc = 1 => ucmem(0) = 0) VARIANT end-pc END));
  List_Substitution(Implementation(assignmentasmw),run)==(VAR pc,end IN pc:=0;end:=1;(uc.init)(end);WHILE pc<end DO BEGIN CASE pc OF EITHER 0 THEN (uc.set)(0,0) END END;pc <-- uc.get_pc END INVARIANT 0<=pc & pc<=end & pc = uc.pc & end = uc.end & (pc = 1 => (uc.mem)(0) = 0) VARIANT end-pc END END)
END
&
...
THEORY ListPropertiesX IS
  ...
  Context_List_Properties(Implementation(assignmentasmw))==(uint32 : POW(INTEGER) & uint32 = 0..MAXINT);
...
THEORY ListVisibleStaticX IS
  List_Constants(Implementation(assignmentasmw),Machine(types))==(uint32);
  List_Constants_Env(Implementation(assignmentasmw),Machine(types))==(Type(uint32) == Cst(SetOf(btype(INTEGER,"[uint32","]uint32"))))
END
&
THEORY ListOfIdsX IS
  List_Of_Ids(Implementation(assignmentasmw)) == (? | ? | ? | ucmem,ucpc,ucend | run | ? | seen(Machine(types)),imported(Machine(uc.ram)) | ? | assignmentasmw);
  List_Of_HiddenCst_Ids(Implementation(assignmentasmw)) == (? | ?);
  List_Of_VisibleCst_Ids(Implementation(assignmentasmw)) == (?);
  List_Of_VisibleVar_Ids(Implementation(assignmentasmw)) == (? | ?);
  List_Of_Ids_SeenBNU(Implementation(assignmentasmw)) == (seen(Machine(types)) : (uint32 | ? | ? | ? | ? | ? | ? | ? | ?));
...
THEORY VariablesLocEnvX IS
  Variables_Loc(Implementation(assignmentasmw),run, 1) == (Type(pc) == Lvl(btype(INTEGER,?,?));Type(end) == Lvl(btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
...
THEORY ImportedVariablesListX IS
  ImportedVariablesList(Implementation(assignmentasmw),Machine(uc.ram))==(uc.mem,uc.pc,uc.end);
  ImportedVisVariablesList(Implementation(assignmentasmw),Machine(uc.ram))==(?)
END
&
