...
THEORY ListOperationsX IS
  Internal_List_Operations(Machine(A8051))==(CJNE,CJNEI,MOV,MOVI,ANL,ACALL,SJMP,JC,JNB,CLR,SETB,AJMP,ADD,ADDD,SUBBS,SUBBD,INC,DEC,INCS);
  List_Operations(Machine(A8051))==(CJNE,CJNEI,MOV,MOVI,ANL,ACALL,SJMP,JC,JNB,CLR,SETB,AJMP,ADD,ADDD,SUBBS,SUBBD,INC,DEC,INCS)
END
&
THEORY ListInputX IS
  ...
  List_Input(Machine(A8051),SUBBS)==(ac,src);
...
THEORY ListHeaderX IS
  ...
  List_Header(Machine(A8051),SUBBS)==(SUBBS(ac,src));
...
THEORY ListPreconditionX IS
  ...
  List_Precondition(Machine(A8051),SUBBS)==(ac : SFR_ADDR & (ac = A or ac = ACC) & src : MEM_ADDR);
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Machine(A8051),SUBBD)==(acc : SFR_ADDR & (acc = A or acc = ACC) & data : WORD | @(result,carry,zero).(result : WORD & result,carry,zero = substract(mem(A),data) & carry : BOOL & zero : BOOL ==> mem:=mem<+{A|->result}) || pc:=INSTRUCTION_NEXT(pc));
  Expanded_List_Substitution(Machine(A8051),SUBBS)==(ac : SFR_ADDR & (ac = A or ac = ACC) & src : MEM_ADDR | @(result,carry,zero).(result : WORD & result,carry,zero = substract(mem(A),mem(src)) & carry : BOOL & zero : BOOL ==> mem:=mem<+{A|->result}) || pc:=INSTRUCTION_NEXT(pc));
...
THEORY ListOfIdsX IS
  List_Of_Ids(Machine(A8051)) == (? | ? | ? | ? | CJNE,CJNEI,MOV,MOVI,ANL,ACALL,SJMP,JC,JNB,CLR,SETB,AJMP,ADD,ADDD,SUBBS,SUBBD,INC,DEC,INCS | ? | seen(Machine(TYPES)),seen(Machine(ALU)),seen(Machine(MEMORY)) | ? | A8051);
