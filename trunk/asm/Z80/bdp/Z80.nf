...
THEORY ListInvariantX IS
  ...
  List_Invariant(Machine(Z80))==(stack : WORD16 --> WORD16 & rgs8 : id_register8 --> WORD8 & mem : WORD16 --> WORD8 & pc : WORD16 & sp : WORD16 & ix : WORD16 & iy : WORD16)
END
&
THEORY ListAssertionsX IS
  ...
  List_Assertions(Machine(Z80))==(dom(stack) = WORD16 & ran(stack) <: WORD16 & ran(mem) <: WORD8 & dom(mem) = WORD16 & ran(rgs8) <: WORD8 & dom(rgs8) = id_register8 & INSTRUCTION_NEXT(0) = 1 & INSTRUCTION_NEXT(1) = 2 & INSTRUCTION_NEXT(2) = 3 & INSTRUCTION_NEXT(3) = 4 & INSTRUCTION_NEXT(4) = 5 & INSTRUCTION_NEXT(5) = 6 & INSTRUCTION_NEXT(6) = 7 & INSTRUCTION_NEXT(7) = 8 & INSTRUCTION_NEXT(8) = 9 & INSTRUCTION_NEXT(9) = 10)
END
&
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Machine(Z80))==(@(rgs8$0).(rgs8$0 : id_register8 -->> WORD8 ==> rgs8:=rgs8$0) || @(mem$0).(mem$0 : WORD16 --> WORD8 ==> mem:=mem$0) || @(pc$0).(pc$0 : WORD16 ==> pc:=pc$0) || @(sp$0).(sp$0 : WORD16 ==> sp:=sp$0) || @(ix$0).(ix$0 : WORD16 ==> ix:=ix$0) || @(iy$0).(iy$0 : WORD16 ==> iy:=iy$0) || @(stack$0).(stack$0 : WORD16 --> WORD16 ==> stack:=stack$0));
  Context_List_Initialisation(Machine(Z80))==(skip);
  List_Initialisation(Machine(Z80))==(rgs8:: id_register8 -->> WORD8 || mem:: WORD16 --> WORD8 || pc:: WORD16 || sp:: WORD16 || ix:: WORD16 || iy:: WORD16 || stack:: WORD16 --> WORD16)
END
&
...
THEORY ListOperationsX IS
  Internal_List_Operations(Machine(Z80))==(CALL,RETURN,LD_r_r_,LD_r_n_,LD_r_HL,LD_HL_r,BIT_b_HL,JP_z_e,SET_b_HL,RES_b_HL,GOTO);
  List_Operations(Machine(Z80))==(CALL,RETURN,LD_r_r_,LD_r_n_,LD_r_HL,LD_HL_r,BIT_b_HL,JP_z_e,SET_b_HL,RES_b_HL,GOTO)
END
&
...
THEORY VisibleVariablesEnvX IS
  VisibleVariables(Machine(Z80)) == (Type(stack) == Mvv(SetOf(btype(INTEGER,"[WORD16","]WORD16")*btype(INTEGER,"[WORD16","]WORD16")));Type(iy) == Mvv(btype(INTEGER,?,?));Type(ix) == Mvv(btype(INTEGER,?,?));Type(sp) == Mvv(btype(INTEGER,?,?));Type(pc) == Mvv(btype(INTEGER,?,?));Type(mem) == Mvv(SetOf(btype(INTEGER,"[WORD16","]WORD16")*btype(INTEGER,"[WORD8","]WORD8")));Type(rgs8) == Mvv(SetOf(etype(id_register8,0,17)*btype(INTEGER,"[WORD8","]WORD8"))))
END
&
THEORY OperationsEnvX IS
  Operations(Machine(Z80)) == (Type(GOTO) == Cst(No_type,btype(INTEGER,?,?));Type(RES_b_HL) == Cst(No_type,btype(INTEGER,?,?));Type(SET_b_HL) == Cst(No_type,btype(INTEGER,?,?));Type(JP_z_e) == Cst(No_type,No_type);Type(BIT_b_HL) == Cst(No_type,btype(INTEGER,?,?));Type(LD_HL_r) == Cst(No_type,etype(id_register8,?,?));Type(LD_r_HL) == Cst(No_type,etype(id_register8,?,?));Type(LD_r_n_) == Cst(No_type,etype(id_register8,?,?)*btype(INTEGER,?,?));Type(LD_r_r_) == Cst(No_type,etype(id_register8,?,?)*etype(id_register8,?,?));Type(RETURN) == Cst(No_type,No_type);Type(CALL) == Cst(No_type,btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
