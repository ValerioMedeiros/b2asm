...
\begin{abstract}
  ...
  implementation thereof.\newline %\keywords{Embedded Software, Simulation, B method and Verification}
...
\item[Power] contains the standard definition of exponentiation, as
  ...
  two, allproved interactively with the Atelier-B prover.
...
registers, etc. The stae transition occurs either as an instruction
...
$\textit{id\_reg16}$. These sets are represented below.
...
are also similar functions for the $\textit{SCHAR}$ type, they are
respectively $\textit{add8SCHAR}$ and $\textit{subtract8SCHAR}$, all
