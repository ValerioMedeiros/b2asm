...
[O que as provas garantem?] As obrigações de provas garante a verficação dos tipos de dados,
das propriedades estabelecidas no modelo e se as expressões são bem formadas \footnote{An expression is called
"well-defined" (or unambiguous) if its definition assigns it a unique interpretation or value.}. As propriedades
oferecem garantias adicionais ao projetista do software, pois é capaz de estabelecer uma série de critérios de
segurança. Além disso, como estas propriedades estão em nível de especificação, então é possível ajusta-las a
necessidade de um determinado sofwtare sem implicar em mudanças no hardware, o que implica apenas em aumentar ou
diminuir o número de provas.
...
distribuição do número de provas de cada grupo. Um dos grupos que contém mais provas é o de manipulação de bits. Apesar da grande
quantidade de provas, este grupo foi rapidamente provado, pois ele utiliza vários lemmas providos pela Hardware
Library, o que acontece também o grupo Logic and Arithmetic e a Types Library.
...
[Conclusao] Este projeto tem um grande número provas(2926) e o processo de prova eh uma das etapas que consomem
...
apenas indicam um caminho, ou seja, ele não introduz hipósteses falsas}. Os passos de prova aceleraram bastante o
...
O processo de prova used three computers each one with two cores, o que teoricamente aumentou em seis vezes a
capacidade de processamento. 

Enfim, podemos afirmar que as técnicas citadas foram o que viabilizaram completamente o uso do método B.
Por este ser um método muito rigoroso, então o seu processo de verificação é torna-se dificil. Por outro, lado as
provas foram quebradas ( modularizadas )
  
 
 \section{Conlusitions}
 
[][Enfatizar a vantagem] The traditional don´t allow verify this things and the properties is flexible. The
