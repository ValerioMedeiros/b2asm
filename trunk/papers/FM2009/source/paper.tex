...
a walk to prover make the proof, thus, theses commands cannot introduce false hypotheses.} associated to match
pattern from proof obligations. The match pattern accelerates too much the prove process, because there are many
...
% verification of harware model. The model created express the semantic of functioning of hardware and
% with this model is possible verify the semantic of hardware components. 

There are in the literature of computer science some technics\cite{BHDL_2003,GEMPLUS_99} to model hardware
and the virtual machines using the B method. Then, the B method has been used successfully to model the
operational semantic, however The cost modeling was still too expensive and this paper describes some
techniques to lower the cost modeling.
In general, the researchers are concetrated at model the Java Virtual Machine (JVM), but it is not easy
to specify and become more difficult to write good lemmas, to normalize the functions and to verify the
properties. Then, this technique specify a thing more simple to get better results.  

This strategy allow modularize the project in more two levels (Hardware an Types)
Como nosso projeto é modularizado e formado por dois projetos básicos, então possível geração de código
para descrição de hardware é simples, pois trabalhamos no nível de descrições de hardware e tipos de
dados e suas funções visam ter um baixo acoplamento e alta coesão.

 

 [Java Bytecode 	]
 - Gplus -Preferimos dar um passo mais curto, ao invés de partir diretamente a máquina virtual, porque
 preferimos fundamentar bem os projetos básicos, para assim facilitar o processo de prova.
 Além disso, ter resultados concretos mais rapidamente e ao mesmo tempo aproximar-se da modelagem do
 processadores modernos atuais.
 
 [Uma das grandes vantagens é a evolução suporte ao método B]
 
 A other advantage is the use of news and advanceds funcionalities of B method. The first is
 the paralelization of prove procces and  the second is instensive use of math patterns associated with proof commands.
 
 [AMM]
Verificação formal já vem sendo aplicada com sucesso na verificação \ldots 
Porém os custos eram altos, pois se passou X tempo para modelar \ldots e 

A modelagem do microcontrolador foi construida por um unico designer and consumiu aproximadamente six
month to study the plattaform Z80, model, adjust to simplifi and prove.
 
 \section{Conlusitions}
 
 % Trab. Futuros
 %- Refinar o modelo para gerar através dos  código e apartir deste código criar um
 %interpretador/simulador
 %- Construir compilador
  
 
%  \begin{itemize}
%    Citar ambuiguidades encontradas no manual e eventuais erros encontrados.
%   \item Utilidade o processo de desenvolvimento do hardware:
%   \subitem Ajuda na remoção de eventuais ambiguidades providas de uma descrição informal.
%   \subitem Verificação de propriedades: sobre a manipulação e tipo dos dados.
%   \subitem Documentação formalizada matemáticamente
%   \subitem Possibilidade da geracao de código VHDL apartir do modelo B.
%   \subitem Colaboracao para construção de um simulador/interpretador
% \end{itemize}
 
[I need review the english]
% [intro]
A necessidade comercial para produzir \textit{hardware} e \textit{software} de alta qualidade é sempre
crescente. O métodos B já têm demonstrado sucesso na especificação de sistemas comerciais críticos.

O suporte fornecido pelo método B é bastante adequado, maduro, extensivo e
...
[ O que foi o diferencial ? qual grande vantagem]
[Eficiência da técnica/método B]
...
No contexto da verificação em nível de assembly:
Exemplificar com propriedade de restrição temporal.
