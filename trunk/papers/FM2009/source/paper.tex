...
[O que as provas garantem?] As obrigações de provas garante a verficação dos tipos de dados, das propriedades
...
sofwtare sem implicar em mudanças no hardware, o que implica apenas em aumentar ou diminuir o número de provas.
...
General & 140 & 615  \\
Properties and Assertions & 68 & 169  \\
 \textbf{Total} &  607 &  2319  &  %\textbf{Total} 2926 \\
\end{tabular}
\end{center}

[Conclusao] Este projeto tem um grande número provas e o processo de prova eh uma das etapas que consomem
maior tempo. Para concluir o processo de prova de todo o microcontrolador foi demando um grande tempo,
aproximadamente um mês. Isto foi custoso, porque foi necessário elaborar, retificar e provar várias lemmas dos
projetos de base (Hardware and Types Library). Entretanto, com os projetos básicos mais maduros, torna-se bem mais
rápida a modelagem e verificação de um novo microprocessador\footnote{Actualy, the microprocessors M8051 and
PIC16F432 are under construction}.
 
Com os projetos de base completamente provados, então aproximadamente 89\% das provas não obvias e 40\% da Proofs WD
foram realizadas automaticamente. As demais provas foram realizadas após o uso de alguns passos de
prova\footnote{Passos de prova são comandos que indicam um caminho para o provador concluir a prova, esses comandos
apenas indicam um caminho, ou seja, ele não introduzem hipósteses falsas}. Os passos de prova aceleraram bastante o
processo de prova, pois como existem muitas instruções do microcontrolador com caractéristicas semelhantes e o
atelierb é capaz de re-usar os passos de prova. Então, poucos passos de prova podem ser úteis para uma grande
variedade de provas. Um exemplo é o conjunto de 17 passos de prova que rapidamente ajudou a verificar 99\% de provas
WD. A experiência em adicionar passos de prova também ajudou a reduzir o tempo do processo de prova em mais de
90\%. Isso aconteceu, porque o uso de passos de prova direciona melhor o provador ao fim da prova, então o processo
de prova é mais rápido.

Para reduzir ainda mais o custo de prova, um nova funcionalidade provida pelo atelierB foi utizada, a paralelizacao.
Para isso foram used three computers each one with two cores, o que teoricamente aumentou em seis vezes a
capacidade de processamento.

A special care was taken to represent the model in way simple and accurate. This care also reduced the number of
proofs. Pois, o simples fato de otimizar o uso do operador ($\lover$) diminuiu sensívelmente o número de
obrigações de provas.

Enfim, as técnicas citadas viabilizaram a verificaçao das 2926 obrigacoes de prova. A modularização dos projetos
definiu lemmas básicos que ajudaram a construir um framework completo e simples. Os passos de prova direcionaram a
resoluçao mais eficente das provas. A paralelizacao e cuidado na modelagem aceleraram as provas, porque aumentaram
a capacidade de processameto e reduziram o número de obrigaçoes de provas.  
 
 \section{Conlusitions}

[intro]
A necessidade comercial para produzir \textit{hardware} e \textit{software} de
alta qualidade é sempre crescente. Métodos formais já têm demonstrado sucesso na
especificação de \textit{softwares} comerciais e críticos; protocolos padrão e
projetos de \textit{hardware}.

O suporte fornecido pelo método B é bastante adequado, maduro, extensivo e eficaz
no processo de especificação formal dessas plataformas. Isso vem sendo
perceptível, pois até então, todas as características relevantes das plataformas
foram modeladas de forma apropriada, principalmente as importantes para
verificação em nível de montagem (\textit{assembly}). Por outro lado, uma das
qualidades de destaque do método B é relacionada com a boa capacidade de
verificação automática das ferramentas, pois, na versão estável das modelagens,
do total de $503$ provas dos módulos principais do \textit{PIC}, \textit{8051} e
\textit{Z80} apenas $3$ referentes ao \textit{Z80} foram provadas de forma
semi-automática. Ou seja, desse total de provas menos de $1$\% necessitaram de
uma simples intervenção do projetista para ser concluída.
...
[Enfatizar a vantagem]
[Flexibilidade]
The traditional don´t allow verify this things and the properties is flexible. The
flexbility allow add restrictions about tempo de execuçao and until specifics restrictions about the application of
microcontroller.

This flexibility of create propertis allow create conditions to verify if the software to finish in cert instant.


 
%  Para a modelagen dessa plataforma, essa modelagen contém linhas e $np $
%  provas não obvias. Uma forma de acelerar o processo de prova, foi paralelizar
%  as provas. Isto foi feito em 4 IMacs e consumiu $ tp $ tempo para verificar
%  automaticamente $np - p $. As demais foi necessário uma certa interação com
%  o provador para sua verificação.
  
  
%  Para oferecer uma maior garantia do modelo com a implementação do processador
%  algumas atividades foram realizadas. Primeiramente, tentantamos animar e gerar
%  código respectivamente nas ferramentas ProB e AtelierB. Entratanto, o modelo
%  cotém algumas construções complexas que ainda não são bem suportada por elas.
%  Então, foi realizada uma apresentação sobre a descrição do modelo e foi
%  realizada um nova revisão com 3 pessoas.
 
 
\section{Acknowledges}

This work received supports from ANP (National Agency of Oil) and CNPq. Besides, the researcher Bruno Gomes help in
the revision.
...
% Paper of ALEMÃO PROB ,java virtual machine
