...
CONSTANTS
	maximum
PROPERTIES
	maximum : INTEGER & MAX_BALANCE  = 100
VARIABLES
	balance
INVARIANT
...
public void add (APDU apdu) { 
	byte[] bf;  short cr; byte apdu_state; short res;
	...
	if (apdu_state == STATE_INITIAL) {
		...
		if (!((short) (balance + cr) <= MAX_BALANCE)) {
...
IN
	...
	THEN 
		...
		IF not (sum_short (balance, cr) <= MAX_BALANCE)
		THEN 
			ISOException.throwIt(EX_BALANCE_EXCEEDED)
...
The Java Card API, in its version 2.2.2\footnote{Java Card 2.2.2,
  ...
  http://java.sun.com/javacard/3.0.1/specs.jsp).}, is composed of 14
