...
%\lstset{language=B, caption={B machine Buscard},label=lis:addimplementation}
%\begin{lstlisting}
%	add(apdu) =
%	BEGIN
%		buffer <- - getBuffer();
%		credit <-- cast ( );
%		IF 
%			not(credit >= 0)      !!! byte version
%		THEN
%			ISOException.throwIt(EXCEPTIONS.negative_credit)
%		ELSIF 
%			not(balance + credit <= MAXIMUM)     !!! byte version
%		THEN
%			ISOException.throwIt(EXCEPTIONS.balance_exceeded)
%		ELSE
%		        balance := sum_byte (balance, credit)  
%		END
%	END
%	
%	[[[[BRUNO e SIMONE: isso esta todo errado - coloquei aqui mais ou menos o que eu acho que deve ser a cara da implementacao B que gera o codigo java card. Voces devem ter algum exemplo para recuperar a estrutura certa, nao?]]]]
%	
%\end{lstlisting}
...
The transition into a defensive style of programming is part of the BSmart method, and implemented by the BSmart Full Function component. This component generates a B refinement of the original specification where pre-conditions are transformed into conditionals (IF-THEN-ELSE-END constructs), with the indication of the need to throw the corresponding exception when conditions are not met by input data and states.
...
But all of these transformations rely, at the bottom, on the need to reference Java Card API types, classes and methods on the B level.
