...
that the user proves that, in a machine, all the its initial states are
...
\includegraphics[width=.92\textwidth]{diagramaEstrutural_vertical2.png}
...
verification of 99\% (2295) of proofs WD. A new feature of B environment also
was used, the parallelization. This used four computers each one with two cores and
it increased many times the capacity of data processing. Finally, all of the
...
using the B method. Then, the B method has been used successfully to model the
operational semantic. However the cost of modelling was still expensive and this
...
construction of this model, some ambiguities and errors were
encountered in the official reference~\cite{Z80_manual}. As
...
to safety properties of the microcontroller state.
