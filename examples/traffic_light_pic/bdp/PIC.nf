...
&
THEORY ListSeesX IS
  List_Sees(Machine(PIC))==(TYPES,ALU)
END
&
THEORY ListUsesX IS
  List_Uses(Machine(PIC))==(?)
END
&
...
THEORY VisibleVariablesEnvX IS
  VisibleVariables(Machine(PIC)) == (Type(sp) == Mvv(btype(INTEGER,?,?));Type(stack) == Mvv(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?)));Type(pc) == Mvv(btype(INTEGER,?,?));Type(c) == Mvv(btype(BOOL,?,?));Type(z) == Mvv(btype(BOOL,?,?));Type(w) == Mvv(btype(INTEGER,?,?));Type(mem) == Mvv(SetOf(btype(INTEGER,"[REGISTER","]REGISTER")*btype(INTEGER,"[WORD","]WORD"))))
END
&
