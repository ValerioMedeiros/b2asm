...
THEORY ListVariablesX IS
  ...
  Local_List_Variables(Machine(PIC))==(sp,stack,pc,c,z,w,mem);
  List_Variables(Machine(PIC))==(sp,stack,pc,c,z,w,mem);
  External_List_Variables(Machine(PIC))==(sp,stack,pc,c,z,w,mem)
END
&
...
THEORY ListOfIdsX IS
  List_Of_Ids(Machine(PIC)) == (? | ? | sp,stack,pc,c,z,w,mem | ? | CALL,RETURN,RETLW,ADDLW,ADDWF,SUBLW,SUBWF,ANDLW,ANDLWF,IORLW,IORLWF,XORLW,XORLWF,BCF,BSF,BTFSC,BTFSS,CLRF,CLRW,COMF,DECF,DECFSZ,GOTO,INCF,INCFSZ,MOVLW,MOVF,MOVWF,NOP,ROLF,RORF,SWAP | ? | seen(Machine(TYPES)),used(Machine(ALU)) | ? | PIC);
...
THEORY VariablesEnvX IS
  Variables(Machine(PIC)) == (Type(sp) == Mvl(btype(INTEGER,?,?));Type(stack) == Mvl(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?)));Type(pc) == Mvl(btype(INTEGER,?,?));Type(c) == Mvl(btype(BOOL,?,?));Type(z) == Mvl(btype(BOOL,?,?));Type(w) == Mvl(btype(INTEGER,?,?));Type(mem) == Mvl(SetOf(atype(REGISTER,"[REGISTER","]REGISTER")*btype(INTEGER,"[WORD","]WORD"))))
END
&
