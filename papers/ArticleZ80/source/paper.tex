...
The rigorous development of an embedded software in a microcontroller requires 
a specification of its instruction set and the software requirements specification. 
The actual industrial practice is to report the instruction set informally using natural language
or with added semi-formal notation. This practice may leave space for errors and inconsistencies \cite{springerlink:Yuan2011}.
This paper presents specifically a B formal model of the Z80 instruction set. 
This model  can be  used on documentation, simulation, test and  a model-driven approach
to design and develop software from the functional specification level down to assembly.
%Using this model-driven approach and the instruction set model, a small case study to
%analyse a petroleum production test system in oil field was developed.
%  
...
\hline
 ...
 BIT & 0..1 & 1 bit &   118 & Hardware \\\hline %49 + 69 = 118 \\\hline
 BYTE & -- & 1 bytes &  153 & Hardware \\\hline%17 + 136 = 153\\\hline
 BV16 & -- & 2 bytes &  75 & Hardware \\ \hline%5 + 70 = 75 \\ \hline
 UCHAR & 0..255 & 1 byte &  30 & Types \\\hline %4 + 26 = 30 \\\hline
 SCHAR & -128..127 & 1 byte & 26 & Types \\\hline %2 + 24 = 26 \\\hline
 USHORT & 0..65.535 & 2 byte & 62 & Types \\\hline % 4 + 58 = 62 \\\hline
 SSHORT & -32.768..32.767 & 2 bytes & 58 & Types \\\hline %  2 + 56 = 58\\\hline
...
using the AtelierB provers.  By default, AtelierB~\cite{atelierB} does
...
properties in the \textit{ASSERTIONS\/} clause, that cause
corresponding proof obligations to be generated and verified. However,
we have not been able to build the corresponding proofs using the base
of rules included in the free distribution of AtelierB.

A solution would be to carry out that verification with ProB
\cite{proB}, as it is able to verify the type of functions, but its
support is generally only suitable for finite functions. Since these
functions use the concept of sequence (an infinite set), we have not
succeded in showing type correctness of the conversion functions with
ProB\marginpar{Estou perdido: não definiu tipos especiais BYTE e BV16
  para poder usar o ProB?}. Eventually, we formalized new proof rules
to check these properties in AtelierB and used them in the interactive
prover of AtelierB.

The hardware library has many conversion functions with
arithmetic,\marginpar{Não seria interessante ter colocado
  esta figura mais cedo? Na seção 2.5, por exemplo? E no texto da
  seção 2.6 poderia enumerar os poucos casos verificados sem regras
  adicionais.}  for example, Bit\_vector to Natural, Natural to
...
%and they have the mean: \textit{BYTE} is a bit vector with eight elements enumerated
...
\textbf{We needed to verify the type of these conversion
  functions}.\marginpar{Não precisa repetir
  isto.} % There are twenty rules associated this properties
...
automatically with the AtelierB: (\it uchar\_schar\rm \ $\in$
...
Finally, four proof obligations are using rules to demonstrate that
the functions are finite\marginpar{Qual o objetivo de provar que as funções são finitas?}
: \it byte\_uchar $ \in \finset$\rm (\it
byte\_uchar\rm ); \it uchar\_byte $ \in \finset$\rm (\it
uchar\_byte\rm ); \it ushort\_bv16 $ \in \finset$\rm (\it
ushort\_bv16\rm ) and \it bv16\_ushort $ \in \finset$\rm (\it
bv16\_ushort\rm ).
\begin{enumerate}
\item  The proof obligation (\it byte\_uchar $\in$   $\finset$ \rm (\it byte\_uchar\rm )) and its 
expansion in rule format is\marginpar{Não entendi esta expansão.}:\\
...
We have included twenty four rules related to binary arithmetic
as\marginpar{Qual a motivação para provar estas propriedades? Parece
  estar fora do escopo do trabalho apresentado no artigo.}  developed
in~\cite{Leibnizens}. These rules are used in the interactive theorem
prover with binary arithmetic to prove important
properties~\cite{James2010,Dasgupta2006}.
