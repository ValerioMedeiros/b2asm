...
techniques to lower the cost modeling. In general, the researchers are concetrated at model the Java
Virtual Machine (JVM), but it is not easy to specify  it, because it very complex, and become more
difficult to write good lemmas, to normalize the functions and to verify the properties. Then, this
technique specify a model more simple to get better results. Besides, the objectives are to get concret
results fastly and, in the same time, stay near to model of actual processors.
% Re-uso  - modularization
The most of related works are specific to one plataform and its basic definitions cannot be re-used at in
other platforms. The used strategy created a good logic separation about the concepts of modelling. Thus,
the basic projects, the Hardware and Types Library, have functions with a low coupling and high
cohesion, that promove the re-use. Soon, these re-use better too the code generation and prove
process.{{Pare aqui \ssssssss }}
 
 [Uma das grandes vantagens é a evolução suporte ao método B]
 
 A other advantage of this work is the use of news and advanceds funcionalities of B method. The first is
 the paralelization of prove procces and  the second is instensive use of math patterns associated with proof
 commands. In the 
 
 [AMM]
Verificação formal já vem sendo aplicada com sucesso na verificação \ldots Porém os custos eram altos,
pois se passou X tempo para modelar \ldots e
