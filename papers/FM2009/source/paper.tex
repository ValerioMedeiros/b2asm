...
In general, the researchers are concetrated at model the Java Virtual Machine (JVM), but it is not easy to specify 
it, because it very big and complex, and become more difficult to write good lemmas, to normalize the functions and
to verify its properties. Then, this work specify a model more simple to get better results fastly, in the same
time, stay near to model of actual processors.
% Re-uso  - modularization
The most of related works are specific to one plataform\cite{GEMPLUS_99,ammm} and its basic definitions cannot be
re-used at in other platforms. The used strategy created a good logic separation about the concepts of modelling.
Thus, the basic projects, the Hardware and Types Library, have functions with a low coupling and high cohesion, that
promove the re-use. Soon, these re-use better too the code generation, the confiability and prove process.
 
 Others advantages of this work are the use of news and advanceds funcionalities of B method. Normaly, the related
 works don`t enjoy this funcionalities, because these are news and required advanced knowleadge about tools for B
 method. These advantages are the paralelization of prove proccess and the instensive use of math patterns
 associated with proof commands.
 
 [AMM]
Verificação formal já vem sendo aplicada com sucesso na verificação \ldots Porém os custos eram altos, pois se
passou X tempo para modelar \ldots e
...
Quais as desvantagens/problemas com relaçao aos outros?
-The B method is not good to verify all kindy of cararacteristics.
-The our results are not mature, b 
-Actually, we don`t define 
-The B method can verify many diferentes properties of model, but some properties cannot be easy to verify and
model.
 -Some times, the B method is very dificult to model, eventually a other metodology can verify 
