...
&
THEORY ListSeesX IS
  List_Sees(Machine(UCHAR_DEFINITION))==(BYTE_DEFINITION)
END
&
THEORY ListUsesX IS
...
THEORY ListAssertionsX IS
  ...
  Context_List_Assertions(Machine(UCHAR_DEFINITION))==(!bt.(bt: BYTE => size(bt) = 8) & size(BYTE_ZERO) = 8 & BYTE <: BIT_VECTOR & BYTE_ZERO: BIT_VECTOR & first(BYTE_ZERO) = 0 & 8: NATURAL);
  List_Assertions(Machine(UCHAR_DEFINITION))==(2**8-1: INTEGER & 0: INTEGER)
END
&
...
THEORY ListConstantsX IS
  List_Valuable_Constants(Machine(UCHAR_DEFINITION))==(UCHAR,uchar_byte,byte_uchar);
  Inherited_List_Constants(Machine(UCHAR_DEFINITION))==(?);
  List_Constants(Machine(UCHAR_DEFINITION))==(UCHAR,uchar_byte,byte_uchar)
END
&
THEORY ListSetsX IS
...
THEORY ListPropertiesX IS
  ...
  Context_List_Properties(Machine(UCHAR_DEFINITION))==(BYTE_INDEX = 1..8 & REAL_BYTE_INDEX = 0..8-1 & BYTE = {bt | bt: BIT_VECTOR & bv_size(bt) = 8} & BYTE_ZERO: BYTE & BYTE_ZERO = BYTE_INDEX*{0} & byte_bit_get: BYTE*REAL_BYTE_INDEX --> BIT & byte_bit_get = %(bt,ind).(bt: BYTE & ind: REAL_BYTE_INDEX | bt(ind+1)));
  Inherited_List_Properties(Machine(UCHAR_DEFINITION))==(btrue);
  List_Properties(Machine(UCHAR_DEFINITION))==(UCHAR = 0..2**8-1 & byte_uchar: BYTE --> UCHAR & byte_uchar = %v0.(v0: BYTE | 2**7*v0(7)+2**6*v0(6)+2**5*v0(5)+2**4*v0(4)+2**3*v0(3)+2**2*v0(2)+2*v0(1)+v0(0)) & uchar_byte: UCHAR --> BYTE & uchar_byte = byte_uchar~)
END
&
...
THEORY ListANYVarX IS
  List_ANY_Var(Machine(UCHAR_DEFINITION),?)==(?)
END
&
...
THEORY ConstantsEnvX IS
  Constants(Machine(UCHAR_DEFINITION)) == (Type(UCHAR) == Cst(SetOf(btype(INTEGER,"[UCHAR","]UCHAR")));Type(uchar_byte) == Cst(SetOf(btype(INTEGER,"[UCHAR","]UCHAR")*SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?))));Type(byte_uchar) == Cst(SetOf(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?))*btype(INTEGER,"[UCHAR","]UCHAR"))))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
